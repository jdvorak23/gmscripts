...
// @include http://deploy.darkmay.cz/phpmyadmin/*
// @include https://db.cesys.eu/*
// @version  1
...
const searchUrlPart = 'tbl_select.php?'; //'index.php?route=/table/search&'
const exportUrlPart = 'export.php'; // 'index.php?route=/export'
...
async function start() {
    ...
    if ( ! queue.isRunning() && window.location.href.includes(searchUrlPart)) {
        ...
        if (params.has('db') && params.has('table')) {
            ...
            for (const row of form.querySelector('table').tBodies[0].rows) {
                ...
                const exportButton = elem(`<span class="btn btn-primary" style="cursor:default;">Export</a>`);
...
class Queue {
    ...
    isRunning() {
        return this.cacheQueue.index !== null;
    }
    ...
    addTasks() {
        ...
        this.tasks.push(async ()=> {
            ...
            if (this.exportSetting) {
                ...
                window.location.href = `${searchUrlPart}db=${this.exportSetting.db}&table=${this.exportSetting.table}`;
            }
        });
    }
    ...
    async exportData() {
		...
		console.log(document.getElementById('page_content'));
		const successDiv = await getDynamicElement('div.success', document.getElementById('page_content'));
		console.log(successDiv);
		let result = document.getElementById('page_content').querySelector('table[data-uniqueid]');
		console.log(result);
        ...
        if ( ! result) {
            ...
            window.location.href = exportUrlPart;
            return;
        }
